...
#define DEBUG_WALK 0
            ...
            reader_t walker_t::get_section_contents(uint32_t sec) const
            {
                return compile_unit_->get_section(sec)->get_contents();
            }

            void walker_t::seek(reference_t ref)
            {
            ...
            vector<reference_t> walker_t::get_path() const
            {
...
#define RETURN(ret) \
    return (ret)
            ...
            const entry_t *walker_t::move_preorder()
            {
                int r;
                do
                {
            ...
            const entry_t *walker_t::move_next()
            {
            ...
            const entry_t *walker_t::move_down()
            {
                int r = 0;
            ...
            const entry_t *walker_t::move_to(reference_t ref)
            {
                seek(ref);
                int r = read_entry();
                RETURN(r == RE_OK ? &entry_ : 0);
            ...
            const entry_t *walker_t::move_up()
            {
