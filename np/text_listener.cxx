...
void text_listener_t::end()
{
    fprintf(stderr, "np: %u run %u failed\n",
            nrun_, nfailed_);
}

void text_listener_t::begin_job(const job_t *j)
{
    fprintf(stderr, "np: running: \"%s\"\n", j->as_string().c_str());
}

void text_listener_t::end_job(const job_t *j, result_t res)
{
...
void text_listener_t::add_event(const job_t *j __attribute__((unused)),
                                const event_t *ev)
{
